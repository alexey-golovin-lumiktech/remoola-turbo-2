...
export function TabletRow({ eyebrow, primary, badges, cells, maxCells = MAX_CELLS }: TabletRowProps): ReactElement {
  ...
  return (
    <article
      className={cn(
        `rounded-card border border-border bg-linear-to-br from-panel via-panel to-white/1.5 px-4 py-3 shadow-[0_16px_40px_rgba(2,6,23,0.16)] transition hover:-translate-y-px hover:border-white/18`,
      )}
    >
      <div className="min-w-0">
        ...
        <div className="mt-1 wrap-break-word text-sm font-medium text-white/95">{primary}</div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 border-t border-white/8 pt-3 sm:grid-cols-2">
        {padded.map((cell, index) => (
          <div
            className="min-w-0 rounded-2xl border border-white/6 bg-white/2.5 p-3 text-xs leading-5 text-white/65"
            key={index}
          >
            {cell}
          </div>
