...
export function MobileTopChips({ identity, activePath }: MobileTopChipsProps): ReactElement | null {
  ...
  return (
    ...
    >
      <span
        className={cn(
          `inline-flex min-h-11 shrink-0 items-center rounded-pill border border-white/8 bg-white/3 px-3 py-1.5 text-[11px] uppercase tracking-[0.18em] text-white/42`,
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              `inline-flex min-h-11 shrink-0 items-center gap-2 rounded-pill border border-white/8 bg-white/3 px-3 py-1.5 text-xs text-white/64 transition hover:-translate-y-px hover:bg-white/5 hover:text-white/85`,
