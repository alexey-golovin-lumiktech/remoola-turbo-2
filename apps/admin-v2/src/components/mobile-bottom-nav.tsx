...
export function MobileBottomNav({ identity, activePath }: MobileBottomNavProps): ReactElement | null {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-bg/95 px-2 pb-[calc(env(safe-area-inset-bottom,0px)+0.5rem)] pt-2 backdrop-blur-md md:hidden"
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                `hover:border-white/10 hover:bg-white/[0.03] hover:text-white/85`,
                active && `border-cyan-400/24 bg-cyan-500/10 text-cyan-100`,
              )}
            >
