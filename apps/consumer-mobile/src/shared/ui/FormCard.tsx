...
export function FormCard({ title, description, children, footer, className = `` }: FormCardProps) {
  return (
    ...
    >
      {(title || description) && (
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-700 dark:bg-slate-800/50 sm:px-6">
      ...
      {footer && (
        <div className="border-t border-slate-200 bg-slate-50 px-4 py-4 dark:border-slate-700 dark:bg-slate-800/50 sm:px-6">
          {footer}
        </div>
      )}
    </div>
