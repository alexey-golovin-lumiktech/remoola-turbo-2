/**
 * Centralized handling for 401 Unauthorized / session expired (admin app).
...
/** Reset so next 401 is handled (e.g. after SPA navigate to
 * login or after fresh load). Call from login page on mount. */
...
/**
 * Call when any API returns 401. Clears auth cookies via logout API (so httpOnly cookies are removed),
