...
describe(`consumer forgot-password route`, () => {
  ...
  it(`forwards forgot-password requests to the canonical backend path`, async () => {
    ...
    const request = new Request(`https://app.example.com/api/consumer/auth/forgot-password`, {
      method: `POST`,
      headers: {
    ...
    expect(String(mockFetch.mock.calls[0]?.[0])).toBe(
      `https://api.example.com/consumer/auth/forgot-password?appScope=consumer`,
    );
