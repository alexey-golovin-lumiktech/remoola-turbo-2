...
describe(`consumer payment-request send route`, () => {
  ...
  it(`forwards explicit consumer appScope to the backend send endpoint`, async () => {
    const req = new Request(`http://localhost:3001/api/payment-requests/pr_1/send`, { method: `POST` });
    ...
    expect(proxyApiRequest).toHaveBeenCalledWith(
      `https://api.example.com/consumer/payment-requests/pr_1/send?appScope=consumer`,
      req,
