...
describe(`ConsumerAuthService.completeProfileCreationAndSendVerificationEmail`, () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: AuthAuditService, useValue: { recordAudit: jest.fn() } },
        {
          ...
          useValue: {
            validateConsumerAppScope: jest.fn((scope?: string | null) => (scope === `consumer` ? scope : undefined)),
          },
        },
      ],
    }).compile();
  ...
  it(`sends signup verification email when consumer is not yet verified`, async () => {
    ...
    await service.completeProfileCreationAndSendVerificationEmail(consumerId, `consumer`);
  ...
  it(`throws when consumer is missing`, async () => {
    ...
    await expect(service.completeProfileCreationAndSendVerificationEmail(consumerId, `consumer`)).rejects.toMatchObject(
      {
