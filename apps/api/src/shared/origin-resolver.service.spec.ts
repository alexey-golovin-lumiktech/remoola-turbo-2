...
describe(`OriginResolverService`, () => {
  ...
  describe(`validateConsumerCorsOrigin`, () => {
    ...
    it(`should allow localhost mobile origin in development`, () => {
      (envs as any).NODE_ENV = `development`;

      const result = service.validateConsumerCorsOrigin(`http://localhost:3002/path`);
      expect(result).toBe(`http://localhost:3002`);
    });

    it(`should reject localhost mobile origin in production`, () => {
