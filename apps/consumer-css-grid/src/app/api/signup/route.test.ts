...
describe(`signup route`, () => {
  ...
  it(`triggers proxied completion after successful signup`, async () => {
    ...
    expect(String(mockFetch.mock.calls[0]?.[0])).toBe(
      `https://api.example.com/api/consumer/auth/signup?appScope=${CURRENT_CONSUMER_APP_SCOPE}`,
    );
    expect(String(mockFetch.mock.calls[1]?.[0])).toBe(
      `https://api.example.com/api/consumer/auth/signup/consumer-123/complete-profile-creation?appScope=${CURRENT_CONSUMER_APP_SCOPE}`,
    );
