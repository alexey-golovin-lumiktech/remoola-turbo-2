...
describe(`AdminV2Access`, () => {
  it(`grants read-only MVP-1b capabilities to OPS admins`, () => {
    const profile = getAdminV2AccessProfile({ type: `ADMIN` });

    expect(profile.role).toBe(`OPS_ADMIN`);
    expect(profile.capabilities).toEqual(
      expect.arrayContaining([`overview.read`, `verification.read`, `consumers.read`, `audit.read`]),
    );
    ...
    expect(profile.workspaces).toEqual(expect.arrayContaining([`overview`, `verification`, `consumers`, `audit`]));
  });
