...
describe(`StripeReversalScheduler`, () => {
  ...
  it(`selects pending deposit reversals for reconcile as well`, async () => {
    await expect(scheduler.reconcilePendingRefunds()).resolves.toBeUndefined();

    const selectionCall = prisma.$transaction.mock.calls[0];
    ...
    await prisma.$transaction.mock.calls[0][0](selectionTx);

    expect(selectionTx.ledgerEntryModel.findMany).toHaveBeenCalledWith({
      where: {
        type: {
  ...
  it(`keeps rows with effective pending status even when raw ledger status drifted`, async () => {
    pendingStripeIds = [`re_drifted`];
    prisma.$transaction = jest.fn().mockImplementation(async (cb: (tx: unknown) => Promise<void>) => {
      const tx = {
        $queryRaw: jest.fn().mockResolvedValue([{ locked: true }]),
        ledgerEntryModel: {
          findMany: jest.fn().mockResolvedValue([
    ...
    expect(refundsRetrieveMock).toHaveBeenCalledWith(`re_drifted`);
  });
