...
describe(`ConsumerAuthService.requestPasswordReset`, () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: AuthAuditService, useValue: { recordAudit: jest.fn(), checkLockoutAndRateLimit: jest.fn() } },
        {
          ...
          useValue: {
            validateReturnOrigin: jest.fn((origin?: string) => (origin ? `http://127.0.0.1:3003` : undefined)),
  ...
  it(`returns unknown_or_unsupported for emails that do not match a consumer`, async () => {
    ...
    await expect(service.requestPasswordReset(`missing@example.com`, `http://127.0.0.1:3003`)).resolves.toBe(
  ...
  it(`sends Google sign-in guidance for passwordless consumers`, async () => {
    ...
    await expect(service.requestPasswordReset(`google-only@example.com`, `http://127.0.0.1:3003`)).resolves.toBe(
  ...
  it(`creates a reset token and sends the standard email for password-backed consumers`, async () => {
    ...
    await expect(service.requestPasswordReset(`user@example.com`, `http://127.0.0.1:3003`)).resolves.toBe(
    ...
    expect(mailingService.sendConsumerForgotPasswordEmail).toHaveBeenCalledWith({
      email: `user@example.com`,
      forgotPasswordLink: `http://127.0.0.1:3334/api/consumer/auth/forgot-password/verify?token=generated-token&referer=http%3A%2F%2F127.0.0.1%3A3003`,
    });
  });
