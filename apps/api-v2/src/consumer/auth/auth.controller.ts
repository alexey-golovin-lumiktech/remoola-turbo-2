...
import { getCookieClearOptions } from '@remoola/api-types';
...
export class ConsumerAuthController {
  ...
  private getRefreshTokenFromRequest(req: express.Request): string | undefined {
    return getApiConsumerRefreshTokenCookieKeysForRead()
  ...
  private resolveRequestOrigin(req: express.Request): string | undefined {
    return this.originResolver.resolveRequestOrigin(req.headers.origin, req.headers.referer);
  }
  ...
  private ensureCsrf(req: express.Request) {
    ...
    if (hasOriginSignal && !this.resolveRequestOrigin(req)) {
  ...
  async googleOAuthStart(
    ...
    const validatedReturnOrigin = this.validateReturnOrigin(returnOrigin) ?? this.resolveRequestOrigin(req);
