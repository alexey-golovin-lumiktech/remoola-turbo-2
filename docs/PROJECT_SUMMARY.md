...
- **Consumer Mobile** — Mobile-first Next.js app running on port 3002. Shares backend API with desktop consumer app. Enhanced UI library with 49 icon components, IconBadge, PageHeader, SearchInput. Comprehensive test coverage for auth flows. Supports Google OAuth with `returnOrigin` parameter for proper redirect after authentication.
