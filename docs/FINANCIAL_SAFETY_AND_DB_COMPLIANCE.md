...
| Admin reversal               | `AdminV2PaymentReversalService`                                                     | Advisory lock `:reversal`; `CHARGEBACK` validates requester balance inside tx. `REFUND` uses Stripe as source of truth, retries the internal append once on failure, and falls back to webhook-driven reconciliation if Stripe already succeeded.                |
...
**Deploy order:** From the monorepo root, run `yarn db:deploy` for deployment environments or `yarn db:migrate` for local development before deploying the app.
...
- [ ] **Prisma:** `yarn db:generate` from the monorepo root succeeds.
...
- [ ] **TypeScript:** `yarn workspace @remoola/api-v2 typecheck` from the monorepo root succeeds (or run the workspace build).
- [ ] **Tests:** Run payment/webhook/ledger unit and concurrency specs; run E2E (`yarn workspace @remoola/api-v2 test:e2e`) so `critical-updates.e2e-spec.ts` and `consumer-verification.e2e-spec.ts` pass.
