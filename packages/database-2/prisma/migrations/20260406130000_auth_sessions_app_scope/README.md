...
consumer session can carry the canonical app namespace (`consumer-css-grid`)
independently of request headers.
...
- Recommended SQL-level guard: `CHECK (app_scope IN ('consumer-css-grid'))`.

## Safety

- Additive-first migration only.
- No destructive change to `reset_password`.
- Legacy scope aliases remain relevant only for historical compatibility reads,
  not for the current write contract.
