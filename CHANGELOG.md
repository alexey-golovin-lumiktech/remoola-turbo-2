...
- **2026-05-28:**
  ...
  - **Consumer wire-contract enforcement:** Route all 17 consumer controllers (auth: signup/session/password/google-oauth; modules: dashboard, payments, payment-requests, payment-methods manual+stripe, verification, contacts, contracts, documents, exchange, profile, settings) through a shared `toConsumerWireContract(schema, value)` Zod gate backed by `packages/api-types/src/consumer/responses.ts`, replacing silent compile-time drift with a loud, test-covered boundary. HTTP wire shapes are byte-identical; HttpOnly session cookies, `ensureCsrf` on refresh/logout/logout-all, OAuth state/PKCE, `deviceId` cookie contract, append-only ledger writes, and Stripe / money-mutating idempotency scopes are untouched (diff is strictly read-path).
  - **Admin-v2 auth and mutation contract hardening:** Centralize admin-v2 `login`, `forgot-password`, `password-reset`, invitation, and revoke-session request/response shapes — plus admin, consumer, document, and exchange mutation bodies and the `force-execute` / `cancel` scheduled-exchange step-up shapes — in `@remoola/api-types`; enforce `requireValidatedJsonBody(...)` on the invitation-acceptance and token-password-reset proxy routes, and parse auth, revoke-session, consumer note/flag, and document-tag payloads through the shared schemas before upstreaming. Step-up confirmation invariants on sensitive admin exchange actions are preserved via aligned declared/actual wire contracts; api-v2 auth DTOs and admin-v2 controller returns are aligned with the shared types; `overview.recentAdminActions[].createdAt` is now serialized as an ISO string to match the public response contract.
  ...
  - **Consumer response schema expansion:** Expand `packages/api-types/src/consumer/responses.ts` from minimal to full coverage of all consumer endpoints (842 lines) using nominal `ConsumerUuid` / `ConsumerIsoDateTime` / `ConsumerDecimalString` and the existing `ConsumerPaginatedOffsetResponse<T>` / `ConsumerPaginatedLimitOffsetResponse<T>` helpers; collapse local duplicates in `apps/consumer-css-grid/src/lib/consumer-api.types.ts` and `lib/queries/{banking,contacts,contracts,dashboard,documents,exchange,payments,settings}.server.ts` to imports from `@remoola/api-types`, keeping local extensions only where they genuinely extend a shared shape (e.g. `DashboardData extends ConsumerDashboardData`).
  - **Shared admin-v2 wire-contract helper:** Introduce an admin-v2 wire-contract helper and validate consumed admin-v2 read responses (auth sessions, admins, consumers, documents, overview, identity, system) against `@remoola/api-types` with explicit shared return typing; replace local auth schemas in `apps/admin-v2` with shared exports.
  ...
  ### 🧪 Testing
  - **Consumer contract drift guard:** Add `packages/api-types/src/consumer/contracts.test.ts` (915 lines) as a schema-level guard against drift on every consumer response shape, including auth responses; `apps/api-v2` records 1471/1471 unit tests passing and `turbo run typecheck lint test build` clean for api-v2, consumer-css-grid, api-types, and admin-v2 with a green consumer-css-grid Next.js production build (Vercel).
  - **Admin-v2 contract and proxy coverage:** Extend contract tests for the new endpoint-specific mutation aliases and inventory mappings, add admin-v2 proxy and backend assertions for shared auth responses and consumed read surfaces, and adjust api-v2 controller specs to return schema-valid wire payloads under the new response-contract assertions; lint, typecheck, build, test, e2e, schema checks, and the Vercel production guard remain green.
...
- **2026-06-10:**
  ...
  ### 🔐 Security / Production Safety
  - **Admin-v2 payments read-model typing restored after query split:** Narrow `admin-v2-payments.query` helper return types and apply `satisfies Prisma.PaymentRequestModelFindManyArgs` in the list/queue builders so Prisma `select`-based payload inference is preserved, align `listPaymentRequests(...)` with an explicit `Promise<AdminV2PaymentsListRow[]>`, and keep the public `./admin-v2-payments.query` row/column type exports consumed by `AdminV2PaymentsPresenter` unchanged — restoring exact queue/list payload typing without changing payment filters, ordering, or bucket sourcing.
  - **Admin-v2 payment reversal workflow invariants centralized:** Extract existing-reversal status/result shaping, refund-final reuse decisions, and ledger-entry assembly into `admin-v2-payment-reversal-workflow.helpers.ts` and thin `AdminV2PaymentReversalWorkflowService` to ledger-mutation sequencing around advisory locks, amount resolution, and audit writes, preserving `AdminV2PaymentReversalService.createReversal(...)` contract, reversal-kind semantics, advisory lock ordering, ledger mutation ordering, and reversal idempotency semantics.
  - **Consumer Stripe checkout and saved-method drift reduced through pure helpers:** Add `stripe-session.helpers.ts` (checkout redirect shaping, Stripe create-input builders, deterministic checkout-session derivations) and `stripe-saved-method.helpers.ts` (saved-method payment-intent builders, result shaping, Stripe error classification) and thin `ConsumerStripeService` to orchestration over the Stripe SDK and repositories, preserving checkout/setup-intent wire behavior, saved-method invalidation branches, Stripe idempotency keys, denial/deny-path semantics, and payment-request claim/bootstrap invariants.
  - **Consumer Google OAuth callback facade thinning finished:** Expand `consumer-google-oauth-callback.helpers.ts` with callback-local state-cookie, redirect, and next-path orchestration and remove the remaining redirect and fallback branching duplication from `ConsumerGoogleOAuthController`, preserving the `google/start`, `google/callback`, `google/signup-session`, `google/signup-session/establish`, and `oauth/complete` route contracts, OAuth state validation, canonical app-scope routing, production/staging blocking of missing/mismatched state, and the invariant that API auth cookies are issued only in `oauthComplete` — never in the Google callback.
  - **Admin-v2 invitation service thinned with local helpers:** Expand `admin-v2-admin-invitations.helpers.ts` with invitation-local guards, assertions, and result builders, refactor `AdminV2AdminInvitationsService` to retain only JWT, idempotency, transaction, link, and mail orchestration, and remove inline invite/accept validation and response shaping duplication. `POST /api/admin-v2/admins/invite` and `POST /api/admin-v2/auth/invitations/accept` contracts, the `typ=admin_invitation` / `scope=admin_v2` JWT invariants, the `admin-invite:${email}` idempotency scope, and the accept-flow consume-before-create transaction gate are preserved.

  ### 🧪 Testing
  - **consumer-css-grid route grid and shell-shell contract guards:** Add `route-grid-contracts.test.ts` (static, render-free guards reading route source files to assert each handoff-listed grid still imports and uses its named token, preventing accidental re-inlining) and `shell-a11y-contracts.test.ts` (bottom-nav stays `md:hidden`, mobile-nav grid column count tracks `mobileNavHrefs.length`, `ShellClientWrapper` uses the shared `SHELL_MAIN_PADDING_CLASS` and `SHELL_CONTENT_OFFSET_CLASS`, `ShellNav` does not re-inline the `md:w-[248px]` literal). Extend `shell-layout-tokens.test.ts`, `shell-card-tokens.test.ts`, `shell-status.test.ts`, and `shell-indicators.test.tsx` to pin the new tokens and tone classes. After the three section splits, the package suite ends green at 450/450 (was 441/441; +4 contact, +5 contract assertions).
  - **api-v2 payment reversal workflow characterization expanded:** Extend coverage for full-reversed implicit reuse and explicit over-remaining rejection paths alongside the helper extraction, keeping the `AdminV2PaymentReversalService.createReversal(...)` integration spec as the end-to-end characterization suite.
  - **api-v2 module-boundary spec migrated to shared assertion helpers:** Extend `module-boundary-test-helpers.ts` with generic source, controller, and file assertion helpers and migrate `module-boundaries.spec.ts` onto them with local path constants, preserving every existing boundary policy, regex check, allowlist, expected map, and test intent and leaving `boundary-allowlists.ts` data unchanged.
  - **Exchange scheduled section static-probe contracts:** Extend `shared/ui/route-grid-contracts.test.ts` with +4 render-free assertions pinning the new exchange composer/section split — `ExchangeScheduleFormSection` imports `FieldHint` from `exchange-shared`, `ExchangeScheduledList` calls both `formatScheduleStatus` and `formatScheduledSecondaryStatus`, `ExchangeScheduledPagination` preserves the `Previous` / `Next` copy, and `ExchangeScheduledSection` keeps the `exchange-scheduled-section` `data-testid`. Package suite ends green at 454/454 (was 450/450).

  ### 🛠 DevEx
  - **consumer-css-grid Dashboard / Contract / Contact section splits:** Behavior-preserving decomposition of the three remaining oversized shell files into composer-plus-siblings, closing the handoff pack's TD-11 Session G route queue. `DashboardSections.tsx` (505 LOC) becomes a 6-line barrel re-exporting six sibling sections (`DashboardUnavailableBanner`, `DashboardMetricsSection`, `DashboardCurrencyBreakdownSection`, `DashboardVerificationSection`, `DashboardActionCardsSection`, `DashboardMainPanelsSection`). `contract-detail-sections.tsx` (387 LOC) becomes a 38-LOC composer assembling seven sibling components (`ContractDetailHeaderSection`, `ContractDetailMetricsSection`, `ContractDetailTimelineSection`, `ContractDetailPaymentHistorySection`, `ContractDetailRelationshipSummarySection`, `ContractDetailFilesSection`, `ContractDetailActiveWorkflowSection`). `ContactDetailView.tsx` (273 LOC) becomes a 79-LOC composer over six sibling section files, with `formatAddress` shared via the composer and the remaining helpers collocated with their owning section. Sole-caller imports stay byte-unchanged; no JSX, copy, href, prop signature, server action, search-param, test-id, or element-type changed.
  - **consumer-css-grid handoff-pack token and status gaps closed:** Add `shellMainAsideWideMain` (`1.5fr_1fr`) and `shellMainAsideLeftSlight` (`1.35fr_1fr`) to `shell-layout-tokens.ts` to cover the last two inline `xl:grid-cols-[…fr_…fr]` ratios in shell routes; add `shellEmptyStateFaint` (preserves dashboard's `text-(--app-text-faint)` tone) and `shellEmptyStateCompact` (preserves the document-attach modal's `py-8`) to `shell-card-tokens.ts`; add a `danger` tone to `shell-status.ts` (reusing the existing `--app-danger-soft` / `--app-danger-text` CSS vars) and broaden SUCCESS / WARNING sets and introduce a DANGER set for the domain statuses rendered by `StatusPill` (Approved, Verified, Executed, Submitted, Draft, Unverified, Failed, Rejected, Cancelled, Terminated, Declined, Expired, "Rejected by payee", etc.) that previously fell through to neutral. Migrate the dashboard, help, and document-attach modal call sites onto the new tokens.
  - **admin-v2-payments query split into definitions and read builders:** Extract Prisma read-model definitions into `admin-v2-payments.query-definitions.ts` and list/queue `findMany` arg builders into `admin-v2-payments.query-helpers.ts`, thin `AdminV2PaymentsQuery` to read orchestration over `PrismaService` and `Promise.all` queue sourcing, and re-export `AdminV2PaymentsListRow` / `AdminV2PaymentsQueueRow` from `./admin-v2-payments.query` to preserve the current external import surface, public method signatures, route wiring, DTO normalization, presenter mapping, queue bucket result shape, and Prisma filter/ordering/bucket-sourcing semantics.
  - **Exchange scheduled section split into composer + dumb sections:** Decompose the 395-LOC `'use client'` `ExchangeScheduledSection.tsx` into a 228-LOC stateful composer plus 3 hook-free sibling files (`ExchangeScheduleFormSection.tsx` — metric pair, currency selects, amount, execute-at, presets, Clear / Schedule buttons; `ExchangeScheduledList.tsx` — filtered list with per-row status badge and Cancel; `ExchangeScheduledPagination.tsx` — Previous / Next). All 8 hooks (`usePathname`, `useRouter`, `useSearchParams`, `useTransition`, 4× `useState`) stay in the composer in original order; children receive already-wired `() => void` / `(id) => void` callbacks and never see `startTransition`. Both runtime callers (`ExchangeClient.tsx`, `exchange/scheduled/page.tsx`) byte-unchanged; no copy / href / search-param / element-type / `data-testid` / prop signature changes.
  - **admin-v2 payout query split into helpers and presenter:** Extract pagination and cursor query primitives into `admin-v2-payout-query.helpers.ts` and payout list / payout case response shaping (metadata parsing, external-reference fallback, escalation shaping) into `admin-v2-payout.presenter.ts`, thinning `AdminV2PayoutQueryService` to orchestration over repository reads, assignment lookups, and high-value policy resolution. `AdminV2PayoutQueryService` public methods, `AdminV2PayoutsService` and controller contracts, and payout list / payout case response shapes are unchanged; repository query semantics, escalation controls, assignment overlays, and destination linkage invariants preserved.
  - **api-v2 unused exports demoted to module-private:** Drop `export` from `normalizeFlag` (admin-v2 consumer action policy), `buildPaymentRequestListWhere` (admin-v2 payments query helpers), `getExternalReference` / `mapPayoutEscalationState` (admin-v2 payout presenter), `DEFAULT_LIMIT` / `MAX_LIMIT` (admin-v2 payout query helpers — `PAYOUT_TYPES` stays exported), `buildStripeSessionReturnUrl` (consumer Stripe session helpers), and every per-domain schema fragment object in `envs-schema.ts` (`node`, `database`, `nest`, `google`, `jwt`, `authLockout`, `mail`, `stripe`, `aws`, `app`, `ngrok`, `vercel`, `security`, `runtimePolicy`, `businessPolicy`, `consumerActionLog`); also remove `AdminV2PaymentsQueueBuckets` from the `admin-v2-payments.query` re-export list. `ENVIRONMENT` and the row/column type exports stay public. Visibility-only — no behavior, type, or runtime semantics change.
