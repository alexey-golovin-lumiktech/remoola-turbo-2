...
export const getInvoiceInfoTable = (info: InvoiceInfo): string => {
  const rows = detailsSampleKeys
    .map(key => {
      const fromData = info.from?.[key]
      const toData = info.to?.[key]

      return infoTableRowTemplate.replace(/{{fromData}}/g, fromData.toString()).replace(/{{toData}}/g, toData.toString())
    })
    .join(`\n`)
